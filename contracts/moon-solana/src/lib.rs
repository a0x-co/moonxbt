...
pub mod token_auction_program {
    ...
    pub fn initialize_auction(
        ...
        auction_config.program_token_account_bump = *ctx.bumps.get("program_token_account").ok_or(ProgramError::InvalidSeeds)?;
    ...
    pub fn place_bid(
        ...
        require!(clock.unix_timestamp < auction_end_timestamp, AuctionError::AuctionNotActive); // Use AuctionNotActive to match constraint
        ...
        if auction_state.highest_bidder != Pubkey::default() && auction_state.highest_bid_amount > 0 {
            ...
            let prev_bidder_token_acc = Account::<TokenAccount>::try_from(&ctx.accounts.previous_bidder_token_account)?;
            ...
            let cpi_accounts_refund = Transfer {
                ...
                to: prev_bidder_token_acc.to_account_info(), // Use the validated account
    ...
    pub fn finalize_auction(ctx: Context<FinalizeAuction>) -> Result<()> {
        ...
        require!(clock.unix_timestamp >= auction_end_timestamp, AuctionError::AuctionNotEnded);
...
pub struct PlaceBid<'info> {
    ...
    #[account(
        ...
        constraint = auction_state.id == auction_config.current_auction_id @ AuctionError::AuctionNotActive
    )]
    pub auction_state: Account<'info, AuctionState>,
...
pub struct FinalizeAuction<'info> {
    ...
    #[account(
        ...
        bump,
        constraint = current_auction_state.id == auction_config.current_auction_id @ AuctionError::AuctionNotActive,
    ...
    #[account(
        ...
        seeds = [
            b"active_auction".as_ref(),
            &(auction_config.current_auction_id.checked_add(1).ok_or(ProgramError::ArithmeticOverflow)?).to_le_bytes() // Seed for the *next* auction ID
        ],
        bump
    )]
