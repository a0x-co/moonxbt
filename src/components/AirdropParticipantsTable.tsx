...
// Componente para iconos con URLs firmadas
const SignedIcon = ({
  bucketName,
  filePath,
  fallbackSrc,
...
const TASKS: Task[] = [
  ...
  },
  {
    ...
    icon: (
      <SignedIcon
        bucketName="a0x-mirror-storage"
        filePath="assets/farcaster.png"
        fallbackSrc="/assets/farcaster.png"
        alt="Farcaster"
        width={20}
        height={20}
        className="inline-block"
  ...
  },
  {
    ...
    icon: (
      <SignedIcon
        bucketName="a0x-mirror-storage"
        filePath="assets/zora.png"
        fallbackSrc="/assets/zora.png"
        alt="Zora"
        width={20}
        height={20}
        className="inline-block"
...
const ALLOWED_WALLETS = [
  ...
  "0x68472e62719acb63f09affc4dac2d86690b0c676",
...
export function AirdropParticipantsTable() {
  ...
  // Hook para los iconos de redes sociales
  ...
  const platformUsernameMap = useMemo(() => {
    ...
    allParticipants.forEach((p) => {
      // Use allParticipants for the map
  ...
  const searchParticipantsForBulk = useCallback(
    // Renamed for clarity
  ...
  useEffect(() => {
    return () => debouncedUpdatePreview.cancel();
  ...
  const filteredParticipants = useMemo(() => {
    ...
    if (filters.fid) {
      ...
      result = result.filter(
        (p) => p.fid && p.fid.toLowerCase().includes(fidSearch) // Asegurar que p.fid existe
      );
    }
    ...
    if (filters.username) {
      ...
      result = result.filter((p) => {
        ...
        const displayNameMatch =
          p.displayName && typeof p.displayName === "string"
  ...
  const handleBulkUpdate = async () => {
    ...
    try {
      ...
      const usernames = bulkUsernames
        ...
        .filter((u) => u);
      const actualTaskId =
        bulkPlatform === "telegram"
          ? `join-${bulkPlatform.toLowerCase()}`
          : `follow-${bulkPlatform.toLowerCase()}`;
      ...
      const updatePromises = matches.map((p) =>
        handleTaskUpdate(p, taskToUpdate)
      );
  ...
  if (!ready)
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] space-y-4 bg-[#1752F0] p-8 rounded-2xl">
        <p className="text-lg md:text-2xl  text-white/80 text-center">
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] space-y-4 bg-[#1752F0] p-8 rounded-2xl">
        ...
        <Button
          ...
          className="w-full py-2.5 px-4 bg-red-500/40 hover:bg-red-500/60 text-white  text-sm rounded-lg border border-white/10 transition-all duration-300"
        >
          Disconnect
        </Button>
      </div>
  ...
  if (error && !loading)
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] space-y-4 bg-[#1752F0] p-8 rounded-2xl">
        ...
        <Button onClick={() => fetchParticipants()} className="bg-white/20">
          Retry
        </Button>
      </div>
    );
              ...
              <Table>
                ...
                <TableBody>
                  {paginatedParticipants.map(
                    (
                      participant // Use paginatedParticipants
                    ) => (
                      <TableRow
                        key={participant.fid}
                      ...
                      >
                        ...
                        <TableCell>
                          <Link
                            href={`https://warpcast.com/${participant.username}`}
                            target="_blank"
                          ...
                          <div className="flex gap-2 flex-wrap">
                            <TooltipProvider>
                              {TASKS.map((task) => {
                                const taskKey = `${participant.fid}-${task.id}`;
                                const isCompleted =
                                  participant.tasks[task.id]?.completed ||
                                  false;
                                ...
                                if (task.id === "join-telegram")
                                  platformUsername =
                                    participant.tasks[task.id]
                                      ?.telegramUsername || platformUsername;
                                return (
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between mt-4 text-white/90">
                <div className="flex items-center gap-2">
                  ...
                  >
                    <SelectTrigger className="w-[70px] bg-white/10 border-white/20 text-white text-sm">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent className="bg-[#1a237e] border-white/20">
          ...
          <DialogContent className="max-w-3xl bg-[#1752F0] border-white/20 text-white">
            <DialogHeader>
              ...
              <DialogDescription className="text-white/80">
                Paste a list of {bulkPlatform || "platform"} usernames (one per
                line) to mark their tasks as completed. These usernames should
                match the {bulkPlatform || "platform"} usernames linked to their
                Farcaster accounts. Participants with already completed tasks
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-white/90">
                  {bulkPlatform
                    ? `${
                        bulkPlatform.charAt(0).toUpperCase() +
                        bulkPlatform.slice(1)
                      } Usernames`
                    : "Usernames"}
                </label>
                <Textarea
                  ...
                  placeholder={`Paste ${
                    bulkPlatform || "platform"
                  } usernames here (one per line)`}
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-white/50">
                              ...
                              {participant.linkedUsernames?.[bulkPlatform] ||
                                participant.tasks[`follow-${bulkPlatform}`]
                                  ?.telegramUsername ||
                                "N/A"}
                              ]
                            </span>
            ...
            <DialogFooter>
              ...
              >
                {bulkUpdating
                  ? "Updating..."
                  : `Update ${previewChanges.length} Tasks`}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
